...
app.post('/api/appointments', (req, res) => {
    const { owner_name, NIC_Number, service, appointment_date, appointment_time } = req.body;
    
    const sql = `
      INSERT INTO appointments (owner_name, NIC_Number, service, appointment_date, appointment_time)
      VALUES (?, ?, ?, ?, ?)
    `;
    db.query(sql, [owner_name, NIC_Number, service, appointment_date, appointment_time], (err, result) => {
      if (err) {
  ...
  app.put('/api/appointments/:id', (req, res) => {
    ...
    const sql = `
      UPDATE appointments
      SET owner_name = ?, NIC_Number = ?, service = ?, appointment_date = ?, appointment_time = ?
      WHERE id = ?
    `;
    db.query(
      sql,
      [owner_name, NIC_Number, service, appointment_date, appointment_time, id],
      (err, result) => {
        if (err) {
